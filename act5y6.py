...
st.set_page_config(page_title="Crime SF", page_icon=":water_pistol:", layout="wide")
...
colonia = st.sidebar.multiselect(
...
df_selection = df.query(
    "Incident_Year == @año & Incident_Day_of_Week == @semana & Analysis_Neighborhood == @colonia"
)
...
df['Incidente'] = df[['Incident Description', 'Incident Date', 'Incident Code']].astype(str).apply(lambda x: ', '.join(x.dropna()), axis=1)
